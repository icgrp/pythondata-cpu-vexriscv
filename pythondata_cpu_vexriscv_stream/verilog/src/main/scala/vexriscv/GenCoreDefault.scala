...
case class ArgConfig(
  ...
  numReadStreams : Int = 0,
  numWriteStreams : Int = 0,
...
object GenCoreDefault{
  ...
  def main(args: Array[String]) {
    ...
    val parser = new scopt.OptionParser[ArgConfig]("VexRiscvGen") {
      ...
      opt[Int]("numReadStreams")     action { (v, c) => c.copy(numReadStreams = v) } text("Set number of axi read streams, 0 mean no read streams")
      opt[Int]("numWriteStreams")     action { (v, c) => c.copy(numWriteStreams = v) } text("Set number of axi write streams, 0 means no write streams")
    ...
    SpinalConfig.copy(netlistFileName = argConfig.outputFile + ".v").generateVerilog {
      ...
      if(argConfig.numReadStreams != 0) {
        plugins += new StreamrPlugin(streamCount = argConfig.numReadStreams)
      }

      if(argConfig.numWriteStreams != 0) {
        plugins += new StreamwPlugin(streamCount = argConfig.numWriteStreams)
      }
